...
for (i in 1:length(indivs_list)){
  ...
  # predict SSM locations at observed times
  pred_new <- predict(FIT, indiv_telemetry, dt = indiv_telemetry$t)
  
  pred_out <- st_as_sf(x=data.frame(ID=1:length(pred_new@.Data[[2]]), 
                                    x=pred_new@.Data[[2]], y=pred_new@.Data[[3]]), 
                       coords=c('x', 'y'),  crs = projection(pred_new))  %>%
    st_transform(st_crs('EPSG:4326')) %>% 
    st_coordinates() 
  
  pred_out_df <- tibble::rownames_to_column(as.data.frame(pred_out), 'loc') %>%
    rename(location.long = "X", location.lat = "Y") 
  
  indiv_df <- tibble::rownames_to_column(indiv, 'loc') %>%
    dplyr::select(individual.local.identifier, species, stop_id, timestamp, lc, loc)
  
  pred_final <- left_join(indiv_df, pred_out_df) %>% dplyr::select(-loc)  

  # save PREDICT results
  filepathS = paste0("data/simulations/", get_indiv, "_predict.csv")
  write.csv(pred_final, file=filepathS, row.names=F)
...
stop_locs <- read.csv('data/HUGO stopover locations.csv') %>%
  ...
  mutate(timestamp = ymd_hms(timestamp), used = 1, sim = 0)
